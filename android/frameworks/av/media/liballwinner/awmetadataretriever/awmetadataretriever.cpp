...
void AwMetadataRetriever::storeMetadata(void)
{
	...
	if(nStrLen > 0)		
    	mMetaData.add(METADATA_KEY_ALBUM, String8((const char*)mMediaInfo.album));
    ...
    nStrLen = strlen((const char*)mMediaInfo.artist);
	logd("METADATA_KEY_ARTIST: %d", nStrLen);
    if(nStrLen != 0)
        mMetaData.add(METADATA_KEY_ARTIST, String8((const char*)mMediaInfo.artist));
	...
	if(nStrLen > 0)
	    mMetaData.add(METADATA_KEY_AUTHOR, String8((const char*)mMediaInfo.author));
	...
	logd("matadata, title: %d", nStrLen);
    if(nStrLen != 0)
        mMetaData.add(METADATA_KEY_TITLE, String8((const char*)mMediaInfo.title));
