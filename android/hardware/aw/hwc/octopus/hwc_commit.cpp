                    ...
                    for(i = 0; i < DisplayData->layer_num_inused[j]; i++)
                    {
                        if(!commit_data.force_flip[j])
                        {
                            if(DisplayData->hwc_layer_info[j][i].aquirefencefd >= 0)
                            {
                                ...
                                if(ret < 0)
                                {
                                    ALOGD("wait fence timeout (%d)disp(%d)layer(%d)",ret,j,i);
                            ...
                            if(DisplayData->hwc_layer_info[j][i].iscursor)
                            {
                                ...
                                lyr = (DisplayData->hwc_layer_info[j][i].hwc_layer_info.channel*4) + DisplayData->hwc_layer_info[j][i].hwc_layer_info.layer_id;
                                commit_data.hwc_layer_info[j][lyr].info.fb.addr[0] = DisplayData->hwc_layer_info[j][i].hwc_layer_info.info.fb.addr[0];
                                commit_data.hwc_layer_info[j][lyr].info.fb.size[0] = DisplayData->hwc_layer_info[j][i].hwc_layer_info.info.fb.size[0];
                            }
