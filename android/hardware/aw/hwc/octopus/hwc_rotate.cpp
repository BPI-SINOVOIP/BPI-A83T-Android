...
static hwc_cache_t *hwc_video_cache_get(int size, int fd, unsigned int sync_count)
{
    ...
    if(wb_cache->handle != -1 && size < wb_cache->size_buffer)
    {
        if(wb_cache->size_buffer - size > 4096)
        {
            ion_free_buffer(wb_cache->handle);
            wb_cache->handle = -1;
            wb_cache->size_buffer = 0;
        }
    }else{
        if(wb_cache->handle != -1)
        {
            ion_free_buffer(wb_cache->handle);
        }
        wb_cache->handle = -1;
    ...
    if(wb_cache->handle == -1 || wb_cache->size_buffer == 0)
    {
        wb_cache->handle = ion_alloc_buffer(size, ION_HEAP_TYPE_DMA_MASK);
        if(wb_cache->handle == -1)
        {
            ALOGD("alloc err from ION_HEAP_CARVEOUT_MASK");
            wb_cache->handle = ion_alloc_buffer(size, ION_HEAP_SYSTEM_CONTIG_MASK);
            if(wb_cache->handle == -1)
            {
...
bool hwc_layer_to_tr(disp_fb_info* hw_layer, tr_info *tr_info, hwc_cache_t *video_buffer, unsigned int tr)
{
    ...
    addr = ion_get_addr_from_handle(video_buffer->handle);
    if(addr == 0)
    {
...
bool hwc_tr_to_layer(disp_fb_info *hw_layer, tr_info *tr_info, hwc_cache_t *video_buffer)
{
    int i = 0;
...
tr2layer:
        if(!hwc_tr_to_layer(&disp_layer->info.fb, &tr_info, wb_cache))
        {
            goto translat_err;
        }
    }else{
