...
static void dump_displays(HwcDisContext_t *Localctx)
{
	...
	if(Globctx->hwcdebug & LAYER_DUMP)
	{
        ...
        ALOGD("+-----------+----+-----+-----+-----------+-------+----------+----------+----------+----------+----------+----------+----------+----------+----------+");
        ALOGD("+\n"
              "+Type|CH|V| Scalht | Scalwt |PL|       Handle     |      Phyaddr     |  Usage  |  Flags  |Tr|Blend| Format  |        Source Crop      |         Frame Crop      | Reason\n"
              "+----+--+-+--------+--------+--+------------------+------------------+---------+---------+--+-----+---------+-------------------------+-------------------------+-------------\n");
        if(AllLayers == NULL)
        {
            return ;
        ...
        for(i = 0; i < Localctx->numberofLayer; i++)
		{
            ...
            ALOGD("+%4s|%2d|%s|%f|%f|%02x| %016x| %016x| %08x| %08x|%02x| %4x| %08x|[%5d,%5d,%5d,%5d]|[%5d,%5d,%5d,%5d]|%s\n",
...
bool inline hwc_rotate_mem(struct private_handle_t *handle)
{
    return (handle->width * handle->height) < 2173600;
}
        ...
        hwc_display_contents_1_t* list, int hwctype) 
{
    ...
    if (list && list->numHwLayers >= 1) 
    {
        for(j = 0; j < list->numHwLayers; j++) 
        {
            ...
            {
                hwc_format_info(&format_info, handle->format);
#ifdef HWC_1_3
...
static inline bool check_cursor(hwc_layer_1_t *layer, int order, int count)
{
    ...
    return (layer->flags & HWC_IS_CURSOR_LAYER)&&(count-order == 2) && check_cursor_format(handle->format);
}
...
static bool inline hwc_format_info(format_info *format_info, int format)
{
    ...
    format_info->bpp = 32;
    format_info->plannum = 1;
    format_info->plnbbp[0] = 32;
    format_info->planWscale[0] = 1;
    format_info->planHscale[0] = 1;
    format_info->align[0] = HW_ALIGN;
    format_info->swapUV = 0;
    switch(format)
    {
        case HAL_PIXEL_FORMAT_RGBA_8888:
            format_info->format = DISP_FORMAT_ABGR_8888;
        break;
        case HAL_PIXEL_FORMAT_RGBX_8888:
            format_info->format = DISP_FORMAT_XBGR_8888;
        break;
        case HAL_PIXEL_FORMAT_BGRA_8888:
            format_info->format = DISP_FORMAT_ARGB_8888;
        break;
	    case HAL_PIXEL_FORMAT_BGRX_8888:
	        format_info->format = DISP_FORMAT_XRGB_8888;
	    break;
        case HAL_PIXEL_FORMAT_RGB_888:
            format_info->format = DISP_FORMAT_BGR_888;
            format_info->plnbbp[0] = 24;
            format_info->bpp = 24;
            format_info->align[0] = HW_ALIGN;
        break;
        case HAL_PIXEL_FORMAT_RGB_565:
            format_info->format = DISP_FORMAT_RGB_565;
            format_info->plnbbp[0] = 16;
            format_info->bpp = 16;
            format_info->align[0] = HW_ALIGN*2;
        break;
        case HAL_PIXEL_FORMAT_YV12:
            format_info->format = DISP_FORMAT_YUV420_P;
            format_info->bpp = 12;
            format_info->plannum = 3;
            format_info->plnbbp[0] = 8;
            format_info->plnbbp[1] = 8;
            format_info->plnbbp[2] = 8;
            format_info->planWscale[1] = 2;
            format_info->planWscale[2] = 2;
            format_info->planHscale[1] = 2;
            format_info->planHscale[2] = 2;
            format_info->align[0] = YV12_ALIGN;
            format_info->align[1] = YV12_ALIGN / 2;
            format_info->align[2] = YV12_ALIGN / 2;
            format_info->swapUV = 1;
        break;
        case HAL_PIXEL_FORMAT_YCrCb_420_SP:
            format_info->format = DISP_FORMAT_YUV420_SP_VUVU;
            format_info->bpp = 12;
            format_info->plannum = 2;
            format_info->plnbbp[0] = 8;
            format_info->plnbbp[1] = 16;
            format_info->planWscale[0] = 1;
            format_info->planWscale[1] = 2;
            format_info->planHscale[0] = 1;
            format_info->planHscale[1] = 2;
        ...
        break;
        case HAL_PIXEL_FORMAT_AW_NV12:
            format_info->format = DISP_FORMAT_YUV420_SP_UVUV;
            format_info->bpp = 12;
            format_info->plannum = 2;
            format_info->plnbbp[0] = 8;
            format_info->plnbbp[1] = 16;
...
static void resize_crop(rect64 *fb_crop, hwc_layer_1_t *psLayer, int *buffer_w, int *buffer_h)
{
    ...
    handle_w = ALIGN(handle->width, format_info.align[0]);
...
static inline int cal_pre_channel(HwcDisContext_t *Localctx, bool isFB, bool isvideo)
{
    ...
    {
        if(Localctx->unasignedVideo 
            > (psDisplayInfo->VideoCHNum -(Localctx->VideoCHCnt >= 0 ? (Localctx->VideoCHCnt+ 1):0)))
        {
            prevideoch = psDisplayInfo->VideoCHNum -(Localctx->VideoCHCnt >= 0 ? (Localctx->VideoCHCnt+ 1):0) - isvideo;
        }else{
...
bool match_format(layer_info_t *psHwlayer_info, disp_layer_info *layer_info)
{
    ...
    err = hwc_format_info(&format_info, handle->format);
    if(!err)
    {
        layer_info->fb.format = format_info.format;
        stride = ALIGN(handle->width, format_info.align[0]);
        i = 0;
        while(i < format_info.plannum)
        {
            ...
            layer_info->fb.size[i].height =
                handle->height / format_info.planHscale[i];
...
bool sunxi_set(hwc_display_contents_1_t** displays,size_t numDisplays)
{
    ...
    int returnfenceFd[CNOUTDISPSYNC] = {HWC_SYNC_INIT,HWC_SYNC_INIT,HWC_SYNC_INIT,HWC_SYNC_INIT};
    int releasefecefd = -1;
    bool needmergfd = 0;
    ...
    {   
        ...
        if(Localctx->UsedFB && fb_layer->handle == NULL)
        {
            ...
            ALOGD("force flip disp[%d] frame[%d]", ture_disp, Globctx->HWCFramecount);
            continue;
        }
        if(disp < HWC_DISPLAY_VIRTUAL)
        {
            ...
            {
                ALOGD("hwc_setup_layer force flip disp[%d] frame[%d]", ture_disp, Globctx->HWCFramecount);
                DisplayData->force_flip[ture_disp] = 1;
            }else{
...
static int hwc_init_display( void)
{
    ...
    for(DispCnt = 0, needupdate = 0, permanentdisp = 0; DispCnt < Globctx->NumberofDisp; DispCnt++)
    {	
        if(needupdate == 1)
        {
            PsDisplayInfo++;
            ...
            switch(PsDisplayInfo->DisplayType)
            {
                ...
                case DISP_OUTPUT_TYPE_HDMI:
                    ...
                    if(PsDisplayInfo == Globctx->SunxiDisplay)
                    {
                        ...
                        PsDisplayInfo->InitDisplayWidth = get_info_mode(PsDisplayInfo->DisplayMode, WIDTH);
                        PsDisplayInfo->InitDisplayHeight = get_info_mode(PsDisplayInfo->DisplayMode, HEIGHT);
                        ...
                        hwc_hotplug_switch(PsDisplayInfo->VirtualToHWDisplay, 1, DISP_TV_MODE_NUM);
                    }
...
SUNXI_hwcdev_context_t* hwc_create_device(void)
{
    ...
    while(j < VIDEO_ROTATE_COUNT)
    {
        ...
        Globctx->video_cache[j].handle = -1;
    ...
    ALOGD( "Primary Display[%d]: Type:%d Mode:%d  Width:[%d]  Height:[%d] memlimit[%d] List[%d][%d][%d] %p"
